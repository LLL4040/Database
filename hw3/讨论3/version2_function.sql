...
BEGIN
    ...
    SELECT id INTO card_id
    FROM cards
    WHERE id >= (SELECT floor(RAND() * (SELECT MAX(id) FROM cards))) AND rarity = card_rarity 
    ORDER BY id
    LIMIT 1;

    /*SELECT MAX(id),MIN(id) INTO max, min
    FROM cards
    WHERE rarity = card_rarity;*/

    /*SELECT ID INTO card_id
      FROM cards
      WHERE ID >= (RAND()*(max - min) + min) AND rarity = card_rarity
      LIMIT 1;*/
    /*SET card_id = round(RAND()*(max - min) + min);*/
...
SET this_month = MONTH(this_time);

SET this_year = YEAR(this_time);
...
while i < 5 do
  ...
  /*IF(EXISTS(SELECT 1 FROM has_card WHERE player_name = username AND card_id = c)) THEN  
    update has_card set num = num + 1 where player_name= username and card_id = c limit 1;
  ELSE
    INSERT INTO has_card(player_name, card_id, num) values (username, c, 1);
  END IF;*/
    set i = i + 1;
